...
function pollCallbackTable() {
	  if (xmlHttp.readyState == 4) {
	      ...
	      var setting = {
			...
			callback: {
				beforeClick: beforeClicktable,
				onCheck: onChecktable
			}
		  };
      jq.fn.zTree.init(jq("#treeDemotable"), setting, zNodes);
    }
  }
}
function beforeClicktable(treeId, treeNode) {
	var zTree =jq.fn.zTree.getZTreeObj(treeId);
	zTree.checkNode(treeNode, !treeNode.checked, null, true);
	return false;
}

function onChecktable(e, treeId, treeNode) {
	var zTree = jq.fn.zTree.getZTreeObj(treeId);
	...
	document.form.citySeltable.value=v;
	document.form.tabName.value=id;
}
function showMenutable() {
	var citySel="#citySeltable";
	var cityObj = jq(citySel);
	var cityOffset = jq(citySel).offset();
	jq("#menuContenttable").css({left:cityOffset.left+ "px", top:cityOffset.top + cityObj.outerHeight()+ "px"}).slideDown("fast");
	jq("body").bind("mousedown", {}, onBodyDowntable);
}
function hideMenutable() {
	jq("#menuContenttable").fadeOut("fast");
	jq("body").unbind("mousedown", {},onBodyDown);
}
function onBodyDowntable(event) {
	if (!(event.target.id == ("menuBtn") || 
			(event.target.id == "citySeltable") || 
			(event.target.id == "menuContenttable") || 
			(jq(event.target).parents("#menuContenttable")).length>0)) {
		hideMenutable();
	}
}
</script>
...
<form name="form" action="<c:url value='/addSysTabInfo.op'/>" method="post">
...
<table align="center" width="60%" border="1" cellspacing="1" cellpadding="0">
	...
	<tr style="width:100%;" >
	    <td height="30" align="right"  width="30%" class="listhead">表清单：</td>
        <td align="left" class="listhead">
           <input id="citySeltable" name="citySeltable" type="text"  style="width:300px;"  objName="表清单" readonly="readonly" nullable="false" onclick="showMenutable();" value=""/>
           <div id="menuContenttable" class="menuContenttable" style="display:none; position: absolute;">
             <ul id="treeDemotable" class="ztree" style="margin-top:0; width:300px; height: 200px;"></ul>
           </div>
        </td>
    </tr>
	<tr class="body" >
		...
		<td align="left" class="listhead"><spring:bind path="command.colList">
		   <input type="text"  name="<c:out value='${status.expression }'/>" value="<c:out value='${status.value }'/>" style="width:300"  nullable="false" value="<c:out value='${status.value}'/>"/>
		   <font color="red"><c:out value="${status.errorMessage}"></c:out></font></spring:bind>
		</td>
    </tr> 
	<tr class="body">
