...
function goBack(){
	form.action="<c:url value='/getDbTableList.op'/>";
	form.method="get";
	form.submit();	
}
	...
	if (xmlHttp.readyState == 4) {
	     if (xmlHttp.status == 200) {
		     ...
		     for(var i=0;i<runs.length;i++){
   		        ...
   		        if(node.id==values[j]){
   			       node.checked=true;
   		           v +=runs[i].getElementsByTagName("colCode")[0].childNodes[0].nodeValue+",";
   		        }
   	         }
   	         obj[i]=node;
           }
...
function onCheck(e, treeId, treeNode) {
	...
	document.form.colList.value=id;
}
...
<form name="form" action="<c:url value='/updateSysTabInfo.op'/>" method="post">
<spring:bind path="command.key"><input type="hidden" name="<c:out value='${status.expression}'/>" value="<c:out value='${status.value}'/>"/></spring:bind>
...
<table align="center" width="60%" border="1" cellspacing="1" cellpadding="0">
	...
	<tr class="body">
		...
		<td align="left" class="listhead"><spring:bind path="command.scheama">
		    <input type="text" name="<c:out value='${status.expression }'/>"objName="模式名" style="width: 300"  maxlength="20"  value="<c:out value='${status.value }'/>" />
		   <font color="red"><c:out value="${status.errorMessage}"></c:out></font></spring:bind>
	    </td>
	</tr>
	<tr class="body">
		...
		<td align="left" class="listhead"> <spring:bind path="command.tabName">
		   <input type="text" name="<c:out value='${status.expression }'/>"objName="表名" style="width: 300"   maxlength="30"  value="<c:out value='${status.value }'/>" />
		   <font color="red"><c:out value="${status.errorMessage}"></c:out></font></spring:bind>
		</td>
	</tr>
	<tr class="body">
		...
		<td align="left" class="listhead"><spring:bind path="command.tabCnName">
		   <input type="text" name="<c:out value='${status.expression }'/>" objName="表中文名" nullable="false" style="width: 300" maxlength="60" value="<c:out value='${status.value }'/>" />
		   <font color="red"><c:out value="${status.errorMessage}"></c:out></font></spring:bind>
		</td>
	</tr>
	...
	<tr class="body">
		...
		<td align="left" class="listhead"><spring:bind path="command.filerStr">
		   <input type="text" name="<c:out value='${status.expression }'/>"objName="过滤条件" style="width: 300" maxlength="4000" value="<c:out value='${status.value }'/>" />
		   <font color="red"><c:out value="${status.errorMessage}"></c:out></font></spring:bind>
	    </td>
	</tr>
	<tr class="body">
		...
		<td align="left" class="listhead"><spring:bind path="command.fileName">
		   <input type="text" name="<c:out value='${status.expression }'/>"objName="文件名" style="width: 300" maxlength="500" value="<c:out value='${status.value }'/>" />
		   <font color="red"><c:out value="${status.errorMessage}"></c:out></font></spring:bind>
		</td>
	</tr>
	<tr class="body">
